...
fn launch_chrome(user_data_dir: &std::path::Path) -> Result<(Child, u16), String> {
    let chrome_bin =
        find_chrome_binary().ok_or_else(|| "Chrome/Chromium not found on this system".to_string())?;
    ...
    let child = cmd.spawn().map_err(|e| format!("Failed to launch Chrome: {e}"))?;
    Ok((child, port))
}
...
async fn extract_text(ws: &mut WsStream, session_id: &str) -> Result<String, String> {
    let text = evaluate_js(ws, session_id, "document.body ? document.body.innerText : ''").await?;
...
async fn check_ssrf(url_str: &str) -> Option<String> {
    ...
    if let Ok(ip) = host.parse::<std::net::IpAddr>() {
        if is_private_ip(ip) {
            return Some(format!("Blocked: {host} resolves to a private/loopback address"));
        }
    }
...
async fn run() -> Output {
    ...
    let _ = cdp_session_send(&mut ws, &session_id, "Runtime.enable", serde_json::json!({})).await;
